...
BOOST_AUTO_TEST_CASE( TypicalWellFormedLogEntries )
{
    ...
    BOOST_CHECK( hasValidStructure("@PETE<56.89,-17.5,+51.4>12;") );
    BOOST_CHECK( hasValidStructure("@ANDY<2000-01-11T01:10:05Z,56.89,-17.5o38'46'',N,+51.4o13',W>52;") );
}
...
{
    BOOST_CHECK( ! hasValidStructure("@<12,17,587>01;") );
}
...
{
    BOOST_CHECK( ! hasValidStructure("@NUNO<12,17,587>012;") );
    BOOST_CHECK( ! hasValidStructure("@NUNO<12,17,587>1;") );
    BOOST_CHECK( ! hasValidStructure("@NUNO<12,17,587>0123;") );
}

BOOST_AUTO_TEST_CASE( UppercaseHexCharacterOkay )
{
    BOOST_CHECK( hasValidStructure("@NUNO<12,17,587>3C;") );
}

BOOST_AUTO_TEST_CASE( LowercaseHexCharacterOkay )
{
    BOOST_CHECK( hasValidStructure("@NUNO<12,17,587>a2;") );
}

BOOST_AUTO_TEST_CASE( InvalidChecksumCharacters )
{
    BOOST_CHECK( ! hasValidStructure("@NUNO<12,17,587>1G;") );
    BOOST_CHECK( ! hasValidStructure("@NUNO<12,17,587>-2;") );
    BOOST_CHECK( ! hasValidStructure("@NUNO<12,17,587> 1;") );
}

BOOST_AUTO_TEST_CASE( ZeroFieldsIsValid )
{
    BOOST_CHECK( hasValidStructure("@XXXX<>01;") );
}

BOOST_AUTO_TEST_CASE( SingleFieldIsValid )
{
    BOOST_CHECK( hasValidStructure("@XXXX<12>01;") );
}

BOOST_AUTO_TEST_CASE( LargeNumberOfFieldsIsValid )
{
    const std::string commas(1000,','); // 1000 fields
    BOOST_CHECK( hasValidStructure("@XXXX<" + commas + ">98;") );
}
...
{
    BOOST_CHECK( ! hasValidStructure("@1234<77>10;") );
    BOOST_CHECK( ! hasValidStructure("@$%&-<77>10;") );
    BOOST_CHECK( ! hasValidStructure("@A*C,<77>10;") );
}

BOOST_AUTO_TEST_CASE( FormatCodeTooShort )
{
    BOOST_CHECK( ! hasValidStructure("@X<77>10;") );
}

BOOST_AUTO_TEST_CASE( InvalidReservedCharInField )
{
    BOOST_CHECK( ! hasValidStructure("@XXXX<23,<,A>10;") );
    BOOST_CHECK( ! hasValidStructure("@XXXX<23,;,A>10;") );
    BOOST_CHECK( ! hasValidStructure("@XXXX<23,@,A>10;") );
}
